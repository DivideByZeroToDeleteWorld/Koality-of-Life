...
local function CreateStyledButton(parent, width, height, text)
    ...
    button:SetBackdropColor(0.8, 0.4, 0.4, 1)  -- Pastel red background
    button:SetBackdropBorderColor(0.2, 0.2, 0.2, 1)  -- Dark border

    -- Text
    local fontPath, fontOutline = GetGeneralFont()
    local buttonText = button:CreateFontString(nil, "OVERLAY")
    buttonText:SetFont(fontPath, 12, fontOutline)
    buttonText:SetPoint("CENTER")
    buttonText:SetText(text)
    buttonText:SetTextColor(1, 1, 0.6, 1)  -- Pastel yellow text
    button.text = buttonText

    -- Hover effect
    button:SetScript("OnEnter", function(self)
        self:SetBackdropColor(0.9, 0.5, 0.5, 1)  -- Lighter on hover
    end)
    button:SetScript("OnLeave", function(self)
        self:SetBackdropColor(0.8, 0.4, 0.4, 1)  -- Back to normal
    end)
...
local function CreateQueueViewer()
    ...
    -- Close button (styled)
    local closeButton = CreateStyledButton(frame, 100, 30, "Close")
